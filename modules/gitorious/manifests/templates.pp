...
define gitorious::version() {
  ...
  Exec { path => ["/opt/rubies/ruby-${ruby_version}/bin","/opt/ruby-enterprise/bin","/usr/local/bin","/usr/bin","/bin", "/usr/sbin"] }
  ...
  exec {"fetch_gitorious_tag":
    ...
    path => ["/opt/rubies/ruby-${ruby_version}/bin","/usr/local/bin","/usr/bin","/bin", "/usr/sbin"],
