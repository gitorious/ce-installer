...
define gitorious::version() {
  ...
  exec {"fetch_gitorious_tag":
    command => "sh -c 'cd ${gitorious::app_root} && git fetch --tags && git checkout master ; git branch -D ce-installed ; git checkout -b ce-installed $name && touch ${gitorious::deployed_tags_dir}/$name'",
