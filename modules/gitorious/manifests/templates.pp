...
define gitorious::version() {
  ...
  exec { "post_version_upgrade":
    command => "sh -c 'export GIT_SSL_NO_VERIFY=true && cd ${gitorious::app_root} && bundle install --without development && env GIT_SSL_NO_VERIFY=true git submodule update --init --recursive && chown -R git:git db vendor && bin/rake db:migrate && touch $probe'",
    path => ["/opt/rubies/ruby-1.9.3-p448/bin", "/usr/local/bin","/usr/bin","/bin", "/usr/sbin"],
