class gitorious::database {
  ...
  exec {"bundle_install":
    command => "/bin/sh -c '/bin/env BUNDLE_GEMFILE=${gitorious::app_root}/Gemfile bundle install --without development test && touch ${gitorious::app_root}/tmp/bundles_installed'",
